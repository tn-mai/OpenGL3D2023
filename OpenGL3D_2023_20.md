...
個々までのプログラムで、関節にゲームオブジェクトを割り当てられるようになったはずです。実際に試してみましょう`ExplorationScene.cpp`を開き、テスト用のスケルタルメッシュを設定するプログラムに次のプログラムを追加してください。
...
次に`Exploration.cpp`を開き、`Initialize`メンバ関数の定義を次のように変更してください。

```diff
     auto skeletal = engine.Create<GameObject>("joint test", pos);
     skeletal->renderer = std::make_shared<SkeletalMeshRenderer>();
     skeletal->renderer->SetMesh(
-      engine.GetSkeletalMesh("Res/MeshData/human_male.obj"));
+      engine.GetSkeletalMesh("Res/MeshData/OrcShaman/OrcShaman.obj"));
     skeletal->AddComponent<AnimationChanger>();
     auto a = skeletal->AddComponent<Animator>();
```

プログラムが書けたらビルドして実行してください。男性の代わりにオークシャーマンが表示されていたら成功です。

<p align="center">
<img src="images/20_result_3.jpg" width="45%" />
</p>

人間の男性用に用意したスケルトンなのであちこちに変なところはありますが、一応アニメーションはできているようです。それでは、オークシャーマンの合わせてスケルトンとアニメーションを調整しましょう。
...
ただ、オークシャーマンで両腕を真下にさげるのはやりすぎなようです。`ExplorationScene.cpp`を開き、`Initialize`メンバ関数の定義にある`kfArmR`配列と`kfArmL`配列の値を次のように変更してください。
...
プログラムが書けたらビルドして実行してください。両腕が少し開いて、変更前よりも自然な動きに見えていたら成功です。
...
<pre class="tnmai_assignment">
<strong>【課題05】</strong>
好きな3DモデルをダウンロードしてOBJファイルに変換し、スケルトンと歩行アニメーションを設定して表示しなさい。
</pre>
