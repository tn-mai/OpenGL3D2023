...
| フレーム | 0 | 0 | 1 | 1 | 2 |
|:--------:|:-:|:-:|:-:|:-:|:-:|
| バッファA | CPUからコピー | GPUが描画 | CPUからコピー | GPUが描画 | ... |
...
>ダブルバッファはメモリ消費量が少なく、トリプルバッファは描画に時間がかかる場合でも安定して高速化が見込めます。現代のコンピュータには十分なメモリがあるため、一般的にはトリプルバッファが選択されます。
...
最後に、スケルタルメッシュを描画する時に使うSSBOの指定方法を変更します。`DrawGameObject`メンバ関数の定義にあるスケルタルメッシュを描画するプログラムを、次のように変更してください。
