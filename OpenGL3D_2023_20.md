...
なにも問題なさそうに見えますが、実は以下の2つの問題があります。

* コピー完了まで描画が待たされる。
* 描画完了までコピーが待たされる。

これらの待ち時間のあいだCPUやGPUは何もしていない(できない)ため、本来の性能を十分に発揮できません。

この問題を解決するには「ダブルバッファ」または「トリプルバッファ」と呼ばれる技法を使います。この技法は名前の通りバッファを二重または三重に持つことで、コピーするバッファと描画するバッファを分けます。
...
このように、描画とコピーを別のバッファに行うことで、「コピー完了待ち」と「描画完了待ち」をなくしています。CPUとGPUの動作を止める要因がなくなったことで、両者の性能を無駄なく発揮できるのです。

>**【ダブルバッファとトリプルバッファのどちらを使うべきか】**<br>
>トリプルバッファのほうが動作が安定しますが、多くのメモリが必要です。しかし、現代のコンピュータには十分なメモリがあるので、通常はトリプルバッファが選ばれます。
...
+    glClientWaitSync(syncJointMatrix[jointMatrixIndex], GL_SYNC_FLUSH_COMMANDS_BIT, timeout);
+
+    // 同期オブジェクトを削除
...
使い終わった動機オブジェクトは明示的に削除しなくてはなりません(削除しないとメモリリークになります)。同期オブジェクトを削除するには`glDeleteSync`(ジーエル・デリート・シンク)関数を使います。

<p><code class="tnmai_code"><strong>【書式】</strong><br>
void glDeleteSync(削除する動機オブジェクト);
</code></p>

最後に、スケルタルメッシュの描画時に使うSSBOの指定方法を変更します。`DrawGameObject`メンバ関数の定義にあるスケルタルメッシュを描画するプログラムを、次のように変更してください。
...
`AnimationCurve`はアニメーションの詳細を定義します。例えば、「歩くアニメーション」は「右腕の回転」「左腕の回転」「右足の回転」「左足の回転」「胴体の位置」という、5つの
`AnimationCurve`によって定義されます。
...
`jointMatrices`配列と同様に、`jointTransforms`配列のサイズはメッシュの関節数と等しい必要があります。そこで、`SetMesh`メンバ関数の定義に次のプログラムを追加してください。
...
`AnimationClip`クラスの`Evaluate`メンバ関数は、登録された全ての`AnimationCurve`
について`AnimationCurve::Evaluate`メンバ関数を呼び出し、その評価結果を対象オブジェクトに反映します。

>**【位置、回転、拡大率以外をアニメーションさせるには】**<br>
>このプログラムではアニメーションの対象を、位置、回転、拡大率のみとしています。他の要素、例えば「ゲームオブジェクトの色」をアニメーションさせたい場合は
>`AnimationTaget`に色を表すメンバを追加し、色を反映するプログラムを作成します。
...
<p align="center">
<img src="images/20_result_0.jpg" width="45%" />&emsp;<img src="images/20_result_1.jpg" width="45%" /><br>
