...
>実際に、2015年頃までのAMD製GPUでは`256`に設定されています。<br>
>その他のGPUでは`16`や`4`に設定されることが多いようです。`256`は`4`と`16`の倍数なので、常に`256`だと仮定すれば全てのGPUでの動作を保証できます。
...
とはいえ、きちんとしたアニメーションを実装するには、それなりのプログラムを書く必要があります。しかし、まだスケルタルメッシュは作成中です。<br>
そこで、ひとまず`ExplorationScene.cpp`に移動させることにします。
