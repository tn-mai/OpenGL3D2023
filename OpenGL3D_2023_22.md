...
* マイクロファセットBRDFについて説明できる。
* マイクロファセットBRDFを構成する`D`項, `G`項, `F`項について説明できる。
* 法線(ノーマル)マッピングを使う理由を説明できる。
* タンジェント空間について説明できる。
* タンジェント空間の計算にMikkTSpaceを使う理由を説明できる。
...
<p align="center">
<img src="images/22_microfacet_model.png" width="66%" /><br>
</p>

BRDFは「双方向反射率分布関数`Bidirectional Reflectance Distribution Function`(バイディレクショナル・リフレクタンス・ディストリビューション・ファンクション」の略です。

BRDFは「ある方向からの入射光が、別のある方向に出射する比率」を表します。「双方向」という名称は、「入射ベクトルと出射ベクトルの向きを入れ替えても、式の結果が変わらない」ことから来ています。

双方向性を満たす反射率分布関数はすべてBRDFと呼ばれます。そのため、エンジンやライブラリごとにBRDFの計算方法は異なります。今回は`Unreal Engine 4`で採用されている計算方法をベースに作成していきます。
...
& N = 法線ベクトル \cr
\cr
& 拡散反射BRDF = \dfrac{ベースカラー}{\pi} \cr
& 鏡面反射BRDF = \dfrac{D * G * F}{4(N \cdot L)(N \cdot V)} \cr
\cr
& 総反射率 = 拡散反射BRDF * (1 - メタリック) + 鏡面反射BRDF \cr
\end{aligned}
$$

ここで`D`は「法線分布項」、`G`は「幾何遮蔽項」、`F`は「フレネル項」を表します。<br>
`N・L`は入射ベクトルと法線ベクトルの内積、`N・V`は出射ベクトル(視線ベクトル)と法線ベクトルの内積です。
...
\begin{aligned}
H &= normalize(L + N) \cr
D &= \dfrac{ラフネス^4}{\pi * ((N \cdot H)^2 * (ラフネス^4 - 1) + 1)^2} \cr
\cr
k &= \dfrac{(ラフネス + 1)^2}{8} \cr
\cr
G &= \dfrac{N \cdot L}{(N \cdot L)(1 - k) + k} * \dfrac{N \cdot V}{(N \cdot V)(1 - k) + k} \cr
\cr
F0 &= 角度0°のときのフレネル値 \cr
F &= F0 + (1 - F0)(1 - L \cdot H)^5 \cr
\end{aligned}
$$
...
<p align="center">
<img src="images/22_microfacet_roughness.png" width="75%" />
</p>
...
法線分布項を表す関数はいくつか考案されていますが、`Unreal Engine`では2007年に
`Walter`氏が考案した、GGX(ジージーエックス、`ground grass unknown`)という関数が使われています。GGX関数は次のように定義されます。
...
乗数やその他のパラメータ(`-1`や`+1`のこと)は、`Unreal Engine`を使うアーティストがラフネスを直感的に操作できるようにと、試行錯誤によって決められた値です。

それでは、シェーダにGGX関数を定義しましょう。`standard.frag`を開き、ユニフォーム変数の定義の下に`NormalDistributionGGX`(ノーマル・ディストリビューション・ジージーエックス、「GGXによる法線分布」という意味)という名前の関数を追加してください。
...
<p align="center">
<img src="images/22_microfacet_geometric_attenuation.png" width="66%" /><br>
[左:視線が他の微小平面に遮られる 右:光線が他の微小平面に遮られる]
</p>

幾何減衰項(きかげんすいこう)は、 このような微小面の状態を考慮して、光線が視点に到達する比率を算出する関数です。

>幾何減衰項は、英語では`Geometric Attenuation Factor`(ジオメトリック・アテニュエーション・ファクター)または`Masking-Shadowing Function`(マスキング・シャドウイング・ファンクション)といいます。略称の`G`は最初の英語名から取られています。

`Unreal Engine`では、Smith(スミス)氏が1969年に考案した技法をSchlick(シュリック)氏が改良した式をベースに、`Unreal Engine`独自の改良を加えたバージョンが使われています。
...
k &= \dfrac{(ラフネス + 1)^2}{8} \cr
\cr
G0(x) &= \dfrac{N \cdot x}{(N \cdot x)(1 - k) + k} \cr
\cr
G &= G0(L)G0(V)
\end{aligned}
$$
...
入射ベクトルが法線ベクトルと直角に近い場合、光が物体表面をかすめるように通過するので、わずかな凹凸でも光が遮られる確率が高くなります。逆に垂直に近い場合は、ほとんどの光が微小平面に到達します。
...
そこで、先の式では入射ベクトルと視線ベクトルの両方について、`G'`関数を使って遮蔽率を求めます。次に、これら2つの値を乗算して「入射光が視点に到達する比率」を算出します。
...
フレネル方程式は水と空気の境界にかぎらず、あらゆる「界面(異なる物質の境界)」で発生します。リアルな映像表現を行う場合、フレネル方程式を避けて通ることはできません。<br>
しかし、フレネル方程式は複雑な計算を必要とします。

>フレネル方程式は以下のURLで確認できます。<br>
>`https://ja.wikipedia.org/wiki/フレネルの式`

前掲のURLを見てみると分かりますが、リアルタイムに計算させるには計算量が多すぎます。<br>
そこで、多少の違いはあるものの、ある程度同じ反射特性を表現できる「近似式(きんじしき)」が考案されました。

この近似式は、考案者の名前をとって「シュリック(Schlick)の近似式」と呼ばれています。<br>
シュリックの近似式は次のように表されます。
...
それから、BRDFでは表面法線の代わりにハーフベクトルが使われます。これによって、`cosθ`
の部分が「入射ベクトル`L`とハーフベクトル`H`の内積」になり、1.1節で見た以下の式につながります。
...
なお、「角度0°のときのフレネル項の値」は`vec3`で受け取ります。非金属ではRGBによる違いはないので`float`で十分です。しかし、金属の場合はRGBごとに値が異なり、それが金属特有の色合いになるからです。
...
関数名は`SpecularBRDF`(スペキュラ・ビーアールディーエフ)とします。
`FresnelSchlick`関数の定義の下に、次のプログラムを追加してください。
...
鏡面反射BRDFの式の分母`4(N・L)(N・V)`は、`D * G * F`を正規化するための係数です。<br>
この係数によって、`入射光 = 鏡面反射 + 拡散反射`が成立するように鏡面反射BRDFの値を調整します。
...
視線ベクトルは「視点座標 - フラグメントの座標」で求められます。フラグメントの座標は
`inPosition`として定義済みなので、あとは視点座標があればいいわけです。
...
<strong>【課題01】</strong>
<code>standard.vert</code>の<code>cameraPosition</code>ユニフォーム変数の定義を確認しなさい。
もしロケーション番号が異なっていたら、<code>standard.vert</code>側に合わせて変更しなさい。同じ番号の場合は何もする必要はありません。
</pre>

ロケーション番号の消費を抑えるため、ラフネスとメタリックはひとつの`vec2`にまとめて定義することにしました。

>**【alphaCutoffをまとめない理由】**<br>
>`alphaCutoff`とまとめて`vec3`にすることも考えました。しかし、ラフネスとメタリックはマテリアルごとに書き換えますが、`alphaCutoff`は描画全体で見てもあまり変更されません。このように有効期間が大きく異なる変数をまとめてしまうと、有効期間が長い変数を無駄に書き換えることになってしまいます。<br>
>有効期間が大きく異なるユニフォーム変数は、多少ロケーション番号が無駄になるとしても、分けておくことでトータルの書き換え量を減らせます。

### 1.7 鏡面反射BRDFと拡散反射BRDFを計算する関数を定義する

次に、鏡面反射BRDFと拡散反射BRDFをまとめて計算する関数を定義します。<br>
関数名は`CalcBRDF`(カルク・ビーアールディーエフ)とします。`SpecularBRDF`関数の定義の下に、次のプログラムを追加してください。
...
角度0°のときのフレネル値`f0`の計算では、メタリックが`0`、つまり材質が非金属のときに
`0.04`が設定されます。`0.04`は非金属の平均的なフレネル値で、「入射光の4%が鏡面反射になる」ことを意味します。

### 1.9 拡散光の計算をBRDFの計算に置き換える

これまで拡散光だけを計算していた部分を、BRDFの計算で置き換えていきましょう。<br>
まず`theta`変数を削除します。これは`CalcBRDF`関数内で計算しているからです。<br>
これによって、`illuminance`の初期値も変更されます。
...
続いて、ディレクショナルライト(平行光源)の各参考の明るさを計算するプログラムを、
`CalcBRDF`関数で置き換えてください。
...
本テキストでは、`shadow`変数の影響を受けるのは平行光源のみとしています。ポイントライトやスポットライトの影まで計算すると、計算にかなりの時間がかかるためです。
...
球体を正面から見たとき、角度45°境界の内側と外側の見かけの面積は等しくなります。<br>
そこで、電卓で「cos(45°)の5乗」を計算して`0.00021555`を求めました。

>`円の面積 = πr^2`より、半径`cos(45°)`の円の面積は`0.5π`になるため。
...
次に、MTLファイルの構文を解釈して、追加した`roughness`(ラフネス)メンバ変数と
`metallic`(メタリック)メンバ変数に値を設定します。

MTLファイルにおいて、鏡面反射は「RGBごとの鏡面反射率(スペキュラ色)」を表す`Ks`(ケーエス)構文と、「なめらかさ(スペキュラ係数)」を表す`Ns`(エヌエス)構文の2つで定義されます。

とりあえず、これらの構文を読み込みましょう。最初に読み取り用の変数を定義します。
`Mesh.cpp`を開き、`LoadMTL`メンバ関数の定義に次のプログラムを追加してください。
...
となる`N`を返します。例えば引数が`16`のとき、`16`は`2`の`4`乗なので、戻り値は`4`になります。

スペキュラ係数の最大値1000は「`2`の10乗」に近いです。そこで、`log2`関数を使ってスペキュラ係数を対数に変換するとだいたい0～10の範囲になります。これを`10`で割ることで、
`0.0`～`1.0`の範囲にしています。

ただし、ラフネスが`0.0`だとBRDFの計算において除数が0になってしまい計算できません。<br>
今回は`clamp`関数を使って、最小値を`0.0001f`とすることで0除算を回避しています。
...
次にメタリックを読み取ります。しかし、メタリックはMTLファイルの標準機能としては存在しません。とはいえ、一部のツールは`Pm`という構文でメタリックを表現しています。

>**【Pmという名称の由来】**<br>
>`m`はメタリック(`metallic`)の頭文字です。<br>
>`P`は物理ベースレンダリング(`Physically Based Rendering`)の頭文字のようです。<br>
...
メタリックやラフネスのように特定のシェーダにしか存在しないユニフォーム変数がある場合、それらを持たないシェーダに対して`glUniform`系の関数を呼び出すと失敗します。
...
このときの結果を記録しておけば、以後は簡単にユニフォーム変数の有無を判定できます。<br>
シェーダに関する機能なので、`ProgramObject`クラスに追加するのが適当でしょう。
...
次に、メッシュの`Draw`関数をラフネスとメタリックに対応させます。<br>
`Mesh.h`を開き、次の先行宣言を追加してください。
...
次に、`Mesh`用の`Draw`関数の定義を次のように変更してください。
...
+void Draw(const StaticMesh& mesh, const ProgramObject& program, const MaterialList& materials);
+void Draw(const std::vector<DrawParams>& drawParamsList, const ProgramObject& program,
...
続いて`Mesh.cpp`を開き、`ProgramObject.h`をインクルードしてください。
...
そして、`Mesh`用の2つの`Draw`関数の定義を次のように変更してください。
 ...
 * 描画パラメータ配列を描画する
 */
-void Draw(const std::vector<DrawParams>& drawParamsList, int program
+void Draw(const std::vector<DrawParams>& drawParamsList, const ProgramObject& program,
   const MaterialList& materials, const vec4* objectColor)
 {
...
それから、`Engine`クラスでこれらの関数を呼び出している部分も変更しなくてはなりません。`Engine.h`を開き、`DrawGameObject`メンバ関数の宣言を次のように変更してください。
...
+  void DrawGameObject(const ProgramObject& prog,
...
+void Engine::DrawGameObject(const ProgramObject& prog,
   GameObjectList::iterator begin,
   GameObjectList::iterator end)
```
...
>**【1章のまとめ】**
>
>* 物体に光が入射すると、その一部は「鏡面反射」し、残りは「拡散反射」する。
...
布地の繊維、アスファルトの凹凸(おうとつ)、水面に立つさざなみなど、現実世界には微細な凹凸が数多く存在します。わたしたちは、こうした物体表面の構造を見慣れているため、凹凸の感じられない物体は作り物だと感じてしまいます。
...
そのため、2Dゲームや初期の3Dゲームでは、これらの凹凸はカラーテクスチャに陰影を描き込むことで再現していました。

しかし、カラーとして描き込まれた陰影は、どの方向から見ても、また、光の向きが変わっても変化しません。そのため、すぐにまやかしの影であることがバレてしまいます。

この問題を解決するために生まれたのが「法線マッピング(またはノーマルマッピング)」です。法線マッピングの基本的な考え方は次のようなものです。

>凹凸が陰影を生むのは法線の方向が異なるからだ。<br>
>ということは、法線さえ再現できれば実際に凹凸を作る必要はないはずだ。

法線の再現には、カラーのかわりに「ある地点における法線の向き」を記録したテクスチャを使います。「法線の向き」を記録したテクスチャのことを「法線(ノーマル)テクスチャ」といいます。

### 2.2 法線マッピングの種類

法線テクスチャに記録する「法線の向き」には、以下の2種類が存在します。

| 名前 | 記録する法線の種類 |
|:-----|:-----|
| オブジェクト空間法線マッピング | モデル座標系(オブジェクト空間)の法線 |
| タンジェント空間法線マッピング | 図形に接する平面を基準とする座標系(タンジェント<br>空間)の法線 |

実装は「オブジェクト空間法線マッピング」のほうが簡単です。そのかわり、「モデルの変形に対応できない」、「テクスチャの再利用が難しい」という欠点があります。

「タンジェント空間法線マッピング」は、計算が複雑になるかわりに「オブジェクト空間法線マッピング」の欠点が解消されています。そのため、多くの法線テクスチャはタンジェント空間用に作られています。

2023年現在のGPUは十分に強力なので、よほどの理由がない限り、多少計算が複雑な以外は欠点のない「タンジェント空間法線マッピング」が使われます。本テキストでも「タンジェント空間法線マッピング」を実装します。
...
<p align="center">
<img src="images/22_tangent_vector.jpg" width="33%" /><br>
[緑=タンジェント(T) 赤=バイタンジェント(B) 青=頂点法線(N)]
</p>

タンジェント空間のX軸は「タンジェントベクトル」と呼ばれます(「タンジェント空間」の名前の由来)。Y軸は「バイタンジェント・ベクトル」と呼ばれます。Z軸は平面の法線で、これは頂点法線(または頂点法線を補間した値)に当たります。
...
この座標系では、テクスチャ座標系のU軸がタンジェントベクトルと一致し、V軸がバイタンジェントベクトルと一致します。「法線テクスチャに記録する、法線ベクトルの基準となる座標系」が選ばれるわけです。
...
>制限があるため汎用性は落ちますが、タンジェントベクトルを頂点に持たせず、シェーダで計算するやり方もあります。
...
>2. 法線テクスチャから法線を読み取り、タンジェント座標系からワールド座標系に変換。
...
>6. 「タンジェントベクトル」と「バイタンジェントベクトルの向き」を計算し、<br>タンジェントベクトル変数に代入。

それでは、頂点シェーダから法線マッピングに対応させていきましょう。<br>
`standared.vert`を開き、法線マッピング用のin変数とout変数を追加してください。
...
タンジェントベクトルは法線と同じ「向きベクトル」です。そのため、平行移動を含まない
`matNormal`行列を使ってワールド座標系に変換します。

### 2.5 skeleta.vertにタンジェントベクトル変数を追加する

3Dモデルの座標変換は`skeletal.vert`でも行っているので、同じ変更を行います。
`skeletal.vert`を開き、法線マッピング用のin変数とout変数を追加してください。
...
しかし、`texNormal`にテクスチャが割り当てられていない場合、`texture`関数の戻り値は
`(0, 0, 0, 0)`になります。結果的に`dot(normal, normal)`も`0`になります。

この`texture`関数の性質を利用して、法線テクスチャが割り当てられている場合のみ法線マッピングを行うようにしています。

それから、法線テクスチャは単なるカラーテクスチャなので、`0`～`1`の値として読み取られます。しかし、実際にはこの値は法線なので、`-1`～`+1`の範囲に変換する必要があります。
...
しかし、この式には「通常のテクスチャでは`0`を表現できない」という問題があります。

これは、通常のテクスチャは8bitなので、0～255までの256段階の値しか記録できないからです。GPUによって0.0～1.0に変換された結果は、以下のようになります。
...
そして、`0`を表現できないことの何が問題かというと、

>頂点法線と正確に一致する法線テクスチャの値が定義できない

ことです。

頂点法線と正確に一致させるには`(0, 0, 1)`という値が必要になります。<br>
しかし、式`t * 2 - 1`では`0`を表現できないのでこの値を再現できません。
...
こうして`-1`～`+1`の値に変換したものが、タンジェント座標系の法線ベクトルになります。この値に`outTBN`行列を乗算すると、ワールド座標系の法線ベクトルが求められます。
...
また、エクスポータによっては`bump`ではなく`Bump`という名前で出力されるため、`scanf`
の機能を利用して`b`と`B`のどちらでも読み込めるようにしています。
...
続いて、頂点データにタンジェントベクトルを追加していきます。`Mesh.h`を開き、`Vertex`
構造体にの定義タンジェントベクトルのメンバ変数を追加してください。
...
同様に、`SkeletalVertex`構造体にもタンジェントベクトルを追加します。
`SkeletalVertex`構造体の定義にタンジェントベクトルのメンバ変数を追加してください。
...
次に、頂点アトリビュートにタンジェントベクトルを追加します。`Mesh.cpp`を開き、
`MeshBuffer`コンストラクタの定義に次のプログラムを追加してください。
...
それから、スケルタルメッシュ用の`LoadOBJ`メンバ関数において、`Vertex`構造体の値を
`SkeletalVertex`構造体にコピーしています。

ここにも、タンジェントベクトルをコピーするプログラムを追加しなくてはなりません。スケルタルメッシュ用の`LoadOBJ`メンバ関数の定義に、次のプログラムを追加してください。
...
このことは、頂点が複数のポリゴンで共有されている場合に問題となります。一般に、

>ひとつの頂点に設定できるタンジェント空間はひとつだけ

だからです。

そのため、法線テクスチャ作成ツールは「頂点が関わる全てのタンジェント空間を平均化」して、頂点のタンジェント空間を作成します。そして、平均化されたタンジェント空間を使って法線テクスチャを作成します。
...
`MikkTSpace`の特徴は、「同じ3Dモデルであれば、たとえインデックス順が異なっていても常に同じ計算結果が得られる」ことです。

それ以外の基本的な部分は「タンジェントとバイタンジェントのなす角が大きいほど、その頂点に影響する面積が広い」という考え方にもとづいています。

MikkTSpaceは計算結果が安定しており、そのうえで品質が高いことから多くの3Dツールやゲームエンジンで採用されています。
...
<p align="center">
<img src="images/22_mikktspace_1.png" width="66%" />
</p>
...
右クリックメニューから「追加」を選び(②)、次に「新しいフィルター(F)」を選択してください(③)。ソリューションエクスプローラーに「新しいフィルター」が追加されるので、名前を
`MikkTSpace`に変更してください。

<p align="center">
<img src="images/22_mikktspace_2.png" width="66%" />
</p>
...
>* `SMikkTSpaceInterface`構造体<br>
>MikkTSpaceシステムとのあいだで、3Dモデルデータを送受信するための関数ポインタを設定する。
>* `SMikkTSpaceContext`構造体<br>
>MikkTSpace実行する`genTangSpaceDefault`(ジェン・タン・スペース・デフォルト)関数の引数。`SMikkTSpaceInterface`構造体へのポインタと、ユーザーデータ(プログラムが任意に設定できるデータ)を設定する。
...
`m_setTSpaceBasic`よりも詳細なタンジェント空間情報を受け取れます。通常の法線マッピングでは詳細情報を使わないので、今回は設定しません。
...
>* `tangent`(タンジェント): タンジェントベクトル
>* `sign`(サイン): バイタンジェントベクトルの向き
>* `face`(フェイス): データを設定するポリゴンの番号
>* `vert`(バート): データを設定する頂点の番号

一般的な法線マッピングの実装では、データ量を節約するためにバイタンジェントではなく「バイタンジェントベクトルの向き」だけを記録します。

バイタンジェントは「タンジェントと法線の両方に垂直」なので、外積によって計算できるからです。

ただし、「タンジェントと法線の両方に垂直」なベクトルは2方向あります。外積で求められるベクトルの向きは決まっているため、本来のバイタンジェントとは逆向きになる場合があります。

そこで、「バイタンジェントベクトルの向き」を記録しておき、正しいバイタンジェントの方向に復元できるようにしています。
...
さて、MikkTSpaceクラスは完成したので、次は適当なタイミングでMikkTSpaceオブジェクトを作成します。オブジェクトを作成するタイミングには、次の2パターンが考えられます。
...
`class MikkTSpace`のように指定すると、先行宣言がなくても型を指定できます。

続いて`Mesh.cpp`を開き、`MeshBuffer`コンストラクタの末尾に次のプログラムを追加してください。
...
>**【2章のまとめ】**
>
>* 法線(ノーマル)マッピングは、テクスチャを利用してポリゴンでは再現しにくい細かいパーツや筋肉の筋などに自然な陰影を付けるための技術。
>* 法線マッピングに使用するテクスチャを「法線テクスチャ」という。法線テクスチャには「法線の向き」が記録される。
>* 「タンジェント空間」は、「頂点座標に接する、頂点法線に垂直な平面を基準とする座標系」。法線マッピングの場合、X軸とY軸は「テクスチャのXY軸方向」、Z軸は「頂点法線の向き」に一致するように定義される。
>* タンジェント空間の軸の向きは計算方法によって異なる。ツールで法線テクスチャを作成するときと、エンジンで法線マッピングを行うときで同じ軸の向きを使わないと、正しい法線が計算できずに表示結果がおかしくなる場合がある。
>* 軸の向きの標準的な計算方法として`MikkTSpace`(ミック・ティー・スペース)という技法が考案され、普及している。
