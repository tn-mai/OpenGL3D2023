...
そこで、`shadow`シェーダの関節行列を扱えるバージョンを作成します。シェーダ名は`shadow_skeletal`(シャドウ・スケルタル)としましょう。

ただ、既に4つのシェーダプログラムがあるので、`shadow_skeletal`シェーダを加えると総数は5になります。数が増えてきたので、この辺りでシェーダプログラムの管理をクラスにまとめておきましょう。

ということで、シェーダプログラムを管理するクラスを作成します。クラス名は`ProgramObject`(プログラム・オブジェクト)とします。
...
次にプロジェクトの`Src/Engine`フォルダに`ProgramObject.cpp`という名前のCPPファイルを追加してください。追加したファイルを開き、次のプログラムを追加してください。
...
+  filenameFS(filenameFS), filenameVS(filenameVS)
+{
+}
+
+// デストラクタ
...
さて、シェーダのコンパイルを行う関数は、すでに`Engine.cpp`に`CompileShader`関数が定義済みです。この関数を切り取って、`ProgramObject.cpp`に貼り付けましょう。`Engine.cpp`を開き、`CompileShader`関数の定義を切り取ってください。
...
>`CompileShader`関数を切り取ったことで`Engine.cpp`にエラーが発生します。これらはあとで修正するので、今は無視してください。
...
さて、これまでシェーダの作成ではコンパイルとリンクを行うだけでした。今回はエラーチェックを追加し、エラーが起きたことを分かりやすくします。`CompileShader`関数に次のプログラムを追加してください。
...
シェーダのコンパイル結果を確認するには、`glGetShaderiv`(ジーエル・ゲット・シェーダ・アイ・ブイ)関数に`GL_COMPILE_STATUS`(ジーエル・コンパイル・ステータス)定数を指定します。

<p><code class="tnmai_code"><strong>【書式】</strong><br>
void glGetShaderiv(シェーダの管理番号, 取得する情報の種類, 情報を格納する変数アドレス);
</code></p>
...
<p><code class="tnmai_code"><strong>【書式】</strong><br>
void glGetProgramiv(プログラムの管理番号, 取得する情報の種類, 情報を格納する変数アドレス);
</code></p>
...
それでは、作成した`ProgramObject`クラスでシェーダプログラムの管理番号を置き換えましょう。`Engine.h`を開き、次の先行宣言を追加してください。
...
+  glProgramUniform1f(*progShadow, locAlphaCutoff, 0.5f); // カットオフ値を設定しておく
   ...
   // FBOを作成
   auto texShadow = std::make_shared<TexHelper>("FBO(depth)", 2048, 2048, GL_DEPTH_COMPONENT32);
   fboShadow = std::make_shared<FramebufferObject>(nullptr, texShadow);
```

カットオフ値を設定するプログラムで、`progShadow`メンバ変数に`*`を付けて「<ruby>参照外<rt>さんしょうはず</rt></ruby>し」を行っています。以前のシェーダ変数は「管理番号」そのものを表す`GLuint`型の値でした。そのため、管理番号を要求する関数に直接渡すことができました。

しかし、今回の変更によって変数の型が「管理オブジェクトへのポインタ」に変わりました。その結果、管理番号を取得するには「参照外し」によってポインタが指す管理オブジェクトを取得する必要が生じたわけです。

シェーダの管理番号はエンジンプログラムの各所で利用しているため、この型の変更によってかなりの数のエラーが発生するはずです。これらのエラー全てについて「参照外し」を追加しなくてはなりません。

練習として、`Render`メンバ関数の先頭付近にあるシェーダ管理変数に「参照外し」を追加してください。
...
>必要な部分を`skeletal.vert`からコピーして追加すると簡単でしょう。
...
`shadow_skeletal`シェーダは、`shadow`シェーダに`skeletal`シェーダの関節行列を扱う機能を追加しただけのものです。

ちょっとした最適化として、「ローカル座標系からワールド座標系に変換」するプログラムにカッコを追加して計算順を制御しています。これが最適化になるのは、`mat4*mat4`よりも`mat4*vec4`のほうが計算回数が少ないためです。

カッコがない場合の式は次のようになります。

`gl_Position = transformMatrix * jointMatrix * vec4(inPosition, 1)`

式は「左から右」に向かって処理されるため、上の式ではまず`transformMatrix * jointMatrix`が計算されます。次に、`transformMatrix * jointMatrix`の結果と`vec4(inPosition, 1)`の乗算が計算されます。

この式では`mat4 * mat4`(乗算64回、加算48回)と`mat4 * vec4`(乗算16回、加算12回)の2つの計算が行われます。

次に、カッコを追加した式は次のようになります。

`gl_Position = transformMatrix * (jointMatrix * vec4(inPosition, 1))`

カッコによって計算順が変更され、最初に`jointMatrix * vec4(inPosition, 1)`が計算されます。この式の結果は`vec4`になります。次に`transformMatrix`と`jointMatrix * vec4(inPosition, 1)`の結果が乗算されます。

このカッコ付きの式では`mat4 * vec4`(乗算16回、加算12回)が2回計算されます。このように、式の計算順を変えるだけで、計算回数を大きく減らせる場合があります。

<pre class="tnmai_assignment">
<strong>【課題02】</strong>
<code>shadow.vert</code>では分かりやすさを優先して<code>transformMatrix * jointMatrix</code>を先に計算しています。もちろん、これは速度面で見ると不利です。
<code>shadow_skeletal.vert</code>を参考に、「ローカル座標系からワールド座標系に変換」するプログラムを高速化しなさい。
</pre>

なお、フラグメントシェーダには`shadow.frag`が流用できます。深度バッファの描画に必要なのはフラグメント座標とテクスチャ座標の2つですが、これは関節行列を使う場合でも使わない場合でも変わらないからです。

### 2.2 shadow_skeletalシェーダを読み込む

それでは`shadow_skeletal`シェーダを読み込みましょう。`Engine.h`を開き、`shadow_skeletal`シェーダ用のメンバ変数を追加してください。
...
`shadow_skeletal`シェーダは`shadow`シェーダと同じフラグメントシェーダ(`shadow.frag`)を使っています。`shadow.frag`には、アルファ値が「カットオフ値」未満のフラグメントを破棄する機能があります。
...
作成した`shadow_skeletal`シェーダを使って、シャドウテクスチャにスケルタルメッシュを描画しましょう。まず、`shadow_skeletal`シェーダのビュープロジェクション行列を設定します。
...
そこで、スケルタルメッシュを記録してスタティックメッシュとは分けて描画することで、時間のかかるシェーダ切り替えを最小限に抑えます。まず記録用の変数を用意しましょう。メッシュを描画するプログラムの先頭に、次のプログラムを追加してください。
...
記録用の変数名は`skeletalMeshList`(スケルタルメッシュ・リスト)としました。次にスケルタルメッシュを記録します。メッシュを描画するプログラムに次のプログラムを追加してください。
...
+    e->renderer->Draw(*e, *progShadowSkeletal, *ssboJointMatrix, bpJointMatrices);
+  }
...
プログラムが書けたらビルドして実行してください。スケルタルメッシュの影が表示されていたら成功です。
...
そこで、同じ方法でカラーの有無を切り替えるようにしてみましょう。`SkeletalMeshRenderer.cpp`を開き、`Draw`メンバ関数の定義に次のプログラムを追加してください。
...
プログラムが書けたらビルドして実行してください。出力ウィンドウにエラーメッセージが表示されなくなっていたら成功です。
...
画像にどんな加工を行うにしても、まずFBOを作成して描画できるようにしないと始まりません。`Engine.h`を開き、3Dモデルを描画するためのFBOメンバ変数を追加してください。
...
ここではひとつのFBOしか追加していませんが、あとでもう少しFBOを追加する予定です。そこで、FBOをまとめて作成するための関数を定義します。
...
改めて処理順を見直してみると、「デフォルトフレームバッファへの描画」に関わる処理が、`1`～`4`と`7`以降に分かれしまっていることに気が付きます。これらは連続した順序にまとめられるはずです。となると、途中の`5`と`6`の処理順を考え直す必要があります。
...
こうしてみると、`5`～`7`を関数の先頭に移動させるだけでよさそうです。それでは、`Render`メンバ関数の定義から以下の範囲を切り取ってください。
...
<p align="center">
<img src="images/21_result_1.jpg" width="45%" /><br>
[影がなくなっている]
</p>
...
このあと、どこかのタイミングで描画先をデフォルトフレームバッファに戻さなくてはなりません。ほとんどの画像加工では、加工したいのは3D描画された画像だけで、UIは加工したくありません。

UIは`overlay`キューで描画しているので、その直前にデフォルトフレームバッファへ戻すことにします。`overlay`キューを描画するプログラムの手前に、描画先をデフォルトフレームバッファに戻すプログラムを追加してください。
...
そのために、画像処理用のシェーダを作成します。このシェーダは、`plane_xy`モデルに`fboMain`のテクスチャを貼り付け、適当に加工して出力するものになります。

`plane_xy`モデルはサイズが±1の正方形なので、座標を変換しないで出力すると、ちょうどビューポート全体を覆うことができます。そのため、頂点シェーダではほとんど何もする必要がありません。

とてもシンプルな頂点シェーダなので、名前は`simple.vert`とします。プロジェクトの`Res`フォルダに`simple.vert`という名前の頂点シェーダを作成してください。作成したファイルを開き、次のプログラムを追加してください。
...
今回は比較的簡単な画像処理として、色変換行列を使用して画像をグレーやセピア色にしてみます。

プロジェクトの`Res`フォルダに`image_processing.frag`という名前のファイルを追加してください。追加したファイルを開き、次のプログラムを追加してください。
...
色変換行列のロケーション番号は、頂点シェーダやフラグメントシェーダと重複しないように`200`としました。
...
追加した変数に画像処理シェーダを読み込みましょう。`Engine.cpp`を開き、`Initialize`メンバ関数の定義に次のプログラムを追加してください。
...
それなりの長さのプログラムになるので、関数として定義します。関数名は`DrawMainImageToDefaultFramebuffer`(ドロー・メイン・イメージ・トゥ・デフォルト・フレームバッファ、「メイン画像をデフォルトフレームバッファに描画」という意味)とします。
...
続いて`DrawMainImageToDefaultFramebuffer`メンバ関数を定義します。`CreateShadowMap`メンバ関数の定義の下に、次のプログラムを追加してください。
...
<p align="center">
<img src="images/21_result_3.jpg" width="45%" />
</p>

色がくすんでしまう原因は、`standard`シェーダと`image_processing`シェーダの両方でガンマ補正を行っていることにあります。そこで、どちらかのガンマ補正を削除する必要があります。
...
それでは、ユニフォーム変数に値を設定して動作を確認しましょう。まずロケーション番号を表す変数を定義しましょう。名前は`locRadialBlur`(ロック・ラジアル・ブラー)とします。`Engine.h`を開き、ロケーション番号の定義を追加してください。
...
そこで、ブラーの開始距離を指定できるようにします。`image_processing.frag`を開き、`radialBlur`ユニフォーム変数に次のコメントを追加してください。
...
次に、縮小ぼかしシェーダを作成します。プロジェクトの`Res`フォルダに`down_samping.frag`(ダウン・サンプリング・フラグ)という名前のテキストファイルを追加してください。追加したファイルを開き、次のプログラムを追加してください。
...
続いて、拡大ぼかしシェーダを作成します。プロジェクトの`Res`フォルダに`up_samping.frag`(アップ・サンプリング・フラグ)という名前のテキストファイルを追加してください。追加したファイルを開き、次のプログラムを追加してください。
...
拡大ぼかしシェーダでは、縮小時よりも大きくぼかします。拡大のほうを大きくするのは、解像度の低いテクスチャほど、読み取りにかかる時間が短くなるためです。

縮小時は解像度の高いテクスチャを読み取って解像度の低いフレームバッファに書き込みます。拡大時はその逆で、解像度の低いテクスチャを読み取って解像度の高いフレームバッファに書き込みます。

そのため、解像度の低いテクスチャを使う拡大時のほうが、テクスチャ読み取りにかかる時間が短くて済みます。この特性を利用して縮小と拡大でぼかしサイズを変えることで、処理時間を抑えながらぼかし品質を高めています。

これで、ブルームエフェクト用のシェーダを準備できました。なお、頂点シェーダには全て`simple.vert`を使用します。

続いて、シェーダファイルを読み込み、縮小コピーを書き込むためのFBOを作成します。`Engine.h`を開き、`Engine`クラスの定義に次のプログラムを追加してください。
...
それでは、シェーダファイルを読み込んでプログラムオブジェクトを作成しましょう。`Engine.cpp`を開き、`Initialize`メンバ関数の定義に次のプログラムを追加してください。
...
今回は、ブルーム用に縮小されたFBOを全部で6個作成しています。インデックス0番のFBOのサイズはメインFBOの縦横1/4、2番目は縦横1/8、3番目は縦横1/16というように、解像度を前のFBOの半分にすることを繰り返します。
...
このプログラムによって、`fboMain`に描画された画像の明るい成分を持つピクセルが`fboBloom[0]`に抽出されます。アルファブレンドは不要なため、無効化している点に注意してください。
...
続いて、縮小した画像をぼかしながら拡大し、それらを加算合成します。これには`up_sampling`シェーダを使います。縮小ぼかしプログラムの下に、次のプログラムを追加してください。
...
それでは、シーンに発生装置を配置しましょう。`ExplorationScene.cpp`を開き、`FireEmitter.h`をインクルードしてください。
...
<pre class="tnmai_assignment">
<strong>【課題08】</strong>
既存のゲームなどを参考に、オリジナルの粒子と粒子発生装置を作成し、シーンに配置しなさい。
あるいは、炎エフェクトを改良しても良いでしょう。発生位置や回転角度などにランダム性を加える、炎だけでなく煙や火の粉を発生させる、などが考えられます。
例:
- ヒットエフェクト
