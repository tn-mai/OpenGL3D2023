...
そこで、スケルタルメッシュを記録してスタティックメッシュとは分けて描画することで、時間のかかるシェーダ切り替えを最小限に抑えます。
...
これで、`color`ユニフォーム変数の有無を取得できるようになりました。<br>
続いて`SkeletalMeshRenderer.cpp`を開き、`Draw`メンバ関数の定義に次のプログラムを追加してください。
