...
ここまでは、OpenGLの機能の説明に集中するために、ほぼすべてのプログラムを`main`関数に書いてきました。プログラムの読みやすさを維持するために、「構造化」を実行する頃合いでしょう。
...
という技法です。C++言語のプログラムを構造化するには、関数、構造体、クラスなどを使います。

この「構造化」をゲームプログラムに活用するために、まずゲームプラグラムの構成を考えてみます。一般的なゲームプログラムは、以下の2つの部分から構成されます。

>* どのゲームでも必要な要素(入出力の制御、画面の描画、ゲームオブジェクトの管理など)
...
+
+  // 衝突が起きたときに呼び出される
+  virtual void OnCollision(const GameObjectPtr& other) {}
+
...
このような「プログラムを管理しやすくするために、クラス内部の仕様を隠す」ことを「カプセル化」といいます。
...
「イベント制御」というコメントの下にある4つのプライベートメンバ関数は、`Engine`クラスから呼び出す予定です。プライベートにしているのは、他のクラスからは呼び出せないようにするためです。

なお、C++のポインタや参照は、C#のように「破棄されたオブジェクトの参照をnullにする」という機能を持ちません。これは`shared_ptr`でも同様です。

そこで、「破棄されたかどうか」を示す`IsDestroyed`(イズ・デストロイド)メンバ関数を定義しています。

>**【パブリックとプライベートの使い分け】**<br>
>メンバ変数の場合、最初はプライベートで作成することをおすすめします。一般に、メンバ変数は「クラスの詳細情報」であり、「うかつに操作されるとバグの元になる」からです。プログラムを書いていくうちに「プライベートメンバだと不便だな」と感じたときに、改めてパブリックにするとよいでしょう。
>メンバ関数は変数の逆で、最初はパブリックで作成します。メンバ関数は「外部からクラスを操作することが目的」の場合が多いためです。
...
+* 衝突イベント
+*
+* @param other 衝突したゲームオブジェクト
+*/
+void GameObject::OnCollision(const GameObjectPtr& other)
+{
+  for (auto& e : components) {
+    e->OnCollision(other);
+  }
...
最後に追加するのは「破棄予定のゲームオブジェクトを削除する」メンバ関数です。関数名は`RemoveDestroyedGameObject`(リムーブ・デストロイド・ゲームオブジェクト)とします。
...
これでゲームオブジェクト管理システムは完成です。ここで一度プログラムをビルドして実行してください。特に問題なく実行できていれば大丈夫です。

<p align="center">
<img src="images/03_result_9.png" width="45%" />
</p>

### 2.12 カメラ操作用のコンポーネントを作成する
