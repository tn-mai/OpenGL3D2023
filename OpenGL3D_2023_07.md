...
>ゲーム会社は作品の「創造性に対する工夫」も見ています。「プレイヤーを楽しませる機会を逃さない人物」を採用したいからです。
>どんなことでもそうですが、創造性は「何度も試してみる」ことで養うことができます。もし「自分には創造性がない」と感じているなら、なおのことチャレンジする回数を増やしましょう。友人やAIに意見を求めてもよいでしょう。
...
>X座標: `-aspectRatio`～`+aspectRatio`(フルHD解像度の場合、約`-1.778`～`+1.778`)<br>
>Y座標: `-1`～`+1`<br>
...
C++でc#のデリゲートに相当する機能を作るには、`function`(ファンクション)型と`vector`型を組み合わせます。
...
また、1で実現するには`if`や`switch`と組み合わせる必要があるため、読みにくいコードになりがちです。しかし、状態の数が少なく、それぞれの状態があまり複雑なことをしない場合は十分役に立ちます。
