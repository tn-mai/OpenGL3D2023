...
UIオブジェクトを作成するプログラムには、ゴール画像を作成するプログラムを流用します。`GoalEvent.cpp`を開き、ゴール画像を作成するプログラムをコピーして、`CreateUIObject`メンバ関数に貼り付けてください。
...
このままだと毎回ゴール画像が作られてしまうので、オブジェクトのパラメータを関数の引数で置き換えます。`CreateUIObject`メンバ関数を次のように変更してください。
...
なお、ゲームオブジェクト名として画像ファイル名を指定している理由は、
...
作成したゲームオブジェクトとコンポーネントは`std::pair`型にまとめられて返されます。`std::pair`は「2つのテンプレート引数をメンバに持つ構造体」です。簡単な定義は次のようになります。
...
>もちろん、`pair`の実際の定義には、標準ライブラリのさまざまな局面で利用される機能が定義されています。

動作テストをかねて、ゴール画像の作成を`CreateUIObject`関数に置き換えます。`GoalEvent.cpp`を開き、ゴール画像を作成するプログラムを次のように変更してください。
...
これまで、メッシュ番号は数値を直接指定してきました。しかし、整数で指定すると、メッシュを追加したり削除したりして、番号が変わることがあると修正に大変な手間がかかります。そこで、番号を表す列挙型を定義することにします。
...
<strong>【課題01】</strong>
コメントに書いてあるように、<code>MeshId</code>(メッシュ・アイディ)列挙型の定義順は、<code>Engine::Initialize</code>メンバ関数にある<code>meshes</code>配列の定義順と一致させなくてはならない。
...
タイトル画面の作成に戻りましょう。まずは初期化を行う`Initialize`メンバ関数を定義します。プロジェクトの`Src`フォルダに`TitleScene.cpp`という名前のCPPファイルを追加してください。追加したファイルを開き、次のプログラムを追加してください。
...
この関数を使ってマウス座標を取得する機能を、ゲームエンジンに追加しましょう。`Engine.h`を開き、`GetKey`メンバ関数の定義の下に次のプログラムを追加してください。
...
それでは、マウスの情報を取得する機能を使って、ボタン機能を持つ`UIButton`(ユーアイ・ボタン)コンポーネントを作成しましょう。`UIButton`コンポーネントは以下の3つの状態を取りうるものとします。
...
C++でc#のデリゲートに相当する機能を作るには、`function`(ファンクション)型と`vector`型を組み合わせます。使い方は、あとで実際に使うときに説明します。
...
このプログラムは、ゲームエンジンからマウス座標`mouse`を取得し、オブジェクトの座標`basePosition`との距離がオブジェクトのスケール未満かどうかを調べます。スケール未満の場合、マウスカーソルはボタンの範囲内にあることになります。
...
左ボタンが離されている場合はさらに、前回更新時のマウスボタンの状態を表す`prevButtonDown`(プレブ・ボタン・ダウン)変数をチェックします。

この変数が`true`の場合、ボタンが離された瞬間のフレームになるため、クリックとみなしてデリゲートを実行します。いずれの場合も、最後に`prevButtonDown`を更新して、次回の更新に備えます。
...
続いて、ボタンがクリックされたときにシーンを切り替える機能を作成します。`TitleScene.cpp`を開き、`UIButton`を作成するプログラムを次のように変更してください。
...
+    "Res/start.tga", { 0, -0.6f }, 0.1f);
+  startButton.second->onClick.push_back([](UIButton* button) {
...
そこで、ゲームエンジンに画面サイズ、アスペクト比を取得する機能を追加します。`Engine.h`を開き、`GetMouseButton`メンバ関数の宣言の下に、次のプログラムを追加してください。
...
関数名は`GetFramebufferSize`(ゲット・フレームバッファ・サイズ)と`GetAspectRatio`(ゲット・アスペクト・レシオ)としました。
...
フェードアウトは時間経過をカウントする必要があります。`TitleScene.h`を開き、`TitleScene`クラスの定義に、次のプログラムを追加してください。
...
```diff
   auto startButton = engine.CreateUIObject<UIButton>(
     "Res/start.tga", { 0, -0.6f }, 0.1f);
-  startButton.second->onClick.push_back([](UIButton* button) {
+  startButton.second->onClick.push_back([this](UIButton* button) {
-    Engine* engine = button->GetOwner()->GetEngine();
-    engine->SetNextScene<MainGameScene>();
+    fadeTimer = 1;
     });
...
次にフェードアウトのためのUIオブジェクトを追加します。`TitleScene.cpp`を開き、`Initialize`メンバ関数の定義に次のプログラムを追加してください。
...
次に`interactable`を使ってボタンの更新を制御するプログラムを追加します。`UIButton.cpp`を開き、`Update`メンバ関数の定義に次のプログラムを追加してください。
...
次に、フェードアウトを制御するための`Update`メンバ関数を定義します。`TitleScene.h`のインクルード文の下に、次のプログラムを追加してください。
...
次に、壁を作成するプログラムに次のプログラムを追加してください。
...
-  player->position = engine.GetMainCamera().position;
...
ダウンロードした<code>skull_mesh.h</code>に含まれる図形データを、<code>Engine::Initialize</code>メンバ関数の<code>meshes</code>配列に追加しなさい。
<code>https://github.com/tn-mai/OpenGL3D2023/tree/main/res/meshdata</code>
</pre>
...
+* @file EnemySkull.h
+*/
+#ifndef ENEMYEYEBALL_H_INCLUDED
+#define ENEMYEYEBALL_H_INCLUDED
...
このため、自分以外のゲームオブジェクトやコンポーネントを参照しなくてもできる初期化は`Awake`で実行し、参照が必要な初期化は`Start`で実行するのが安全です。
...
それでは、作成した敵をマップに配置しましょう。`MainGameScene.cpp`を開き、`EnemySkull.h`をインクルードしてください。
...
このリストでは、下の例ほど設計が抽象的で再利用性が高まりますが、そのぶん実装が複雑になります。このような設計は重要ですが、実装を複雑にしすぎるのもよくありません。そこで、2の「関数オブジェクト」を使う方法を試そうと思います。

関数オブジェクトは「関数のように扱えるオブジェクト」のことです。C++においては「関数呼び出し構文を使えるように`operator()`をオーバーロードしたクラスまたは構造体」を意味します。これにはラムダ式も含まれます。

C++の標準ライブラリには、関数オブジェクトを扱うための`std::function`(ファンクション)というクラステンプレートが用意されています。`std::function`を使うと、関数やラムダ式を、簡単に変数として扱うことができます。

<p><code class="tnmai_code"><strong>【書式】</strong><br>
std::function<戻り型(引数リスト)>;
</code></p>

実際に`std::function`を使ってみましょう。`EnemySkull.h`を開き、次のプログラムを追加してください。
...
`getMapData`(ゲット・マップ・データ)が「関数オブジェクト」を保持する変数です。
...
また、`std::function`型の変数を条件式に指定すると、値が設定されていれば`true`、設定されていなければ`false`になります。上記のプログラムではこれを利用して、設定されている場合のみ移動を実行するようにしています。

ということで、`getMapData`変数に関数オブジェクトを設定しましょう。`MainGameScene.cpp`を開き、`Initialize`メンバ関数に次のプログラムを追加してください。
...
次に、衝突したときに実行される`OnCollision`メンバ関数をオーバーライドします。`Udpate`メンバ関数の定義の下に、次のプログラムを追加してください。
...
画像の準備ができたらゲームオーバーを作成しましょう。ゲームオーバーの管理は`MainGameScene`クラスで行うことにします。そのために、「プレイ中」と「ゲームオーバー」という2つの状態を表す機能を追加します。
...
このように、「状態」を表現する方法はひとつではありません。大きく以下の3種類に分けられるので、状況によって適切なものを選択します。

>1. 状態を「整数型」や「列挙型」で表現する。単純な状態の制御ならこれで十分。
>2. 状態を「関数ポインタ」や「`std::function`」で表現する。「状態ごとに実行する関数を分けたい」、「分けたい関数はひとつだけ」という場合に使う。
>3. 状態を「クラス」で表現する。いわゆるステートパターン。「分けたい関数が複数ある」とか「状態ごとに異なる変数を扱いたい」という場合に使う。

今回は2の方法を使いましたが、理由は「状態ごとに異なる更新関数を実行したい」からです。これは3でも可能ですが、分けたい関数がひとつだけの場合は無駄が多いです。

また、1で実現するには`if`や`switch`と組み合わせる必要があるため、読みにくいコードになりがちです。
...
+  if (playerComponent->GetState() == PlayerComponent::State::gameOver) {
+    // ゲームオーバー画像を表示
