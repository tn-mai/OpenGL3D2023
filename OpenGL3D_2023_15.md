...
+    const float sx = copysign(length(vec3(transform[0])), transform[0].x);
+    const float sy = copysign(length(vec3(transform[1])), transform[1].y);
+    const float sz = copysign(length(vec3(transform[2])), transform[2].z);
...
<p><code class="tnmai_code"><strong>【書式】</strong><br>
float copysign(符号を付ける値, 符号のコピー元になる値);
</code></p>
