...
目標は、高低差のある空間を作成し、その中を自由に移動できるようにすること。
...
+      std::copysign(length(vec3(transform[0])), transform[0].x),
+      std::copysign(length(vec3(transform[1])), transform[1].y),
+      std::copysign(length(vec3(transform[2])), transform[2].z)});
...
### 1.11 MainGameSceneクラスをColliderに対応させる

ここまででゲームエンジンに関する変更は終わりました。次はエンジンを利用している`MainGameScene`クラスを`Collider`クラスに対応させます。
...
プログラムが書けたらビルドして実行してください。衝突判定に多くの変更を加えましたが、とりあえず問題なく動作していれば成功、のはずです。
