...
### 2.12 MeshBufferクラスにFBXファイル読み込み機能を追加する
...
### 2.13 EngineクラスにFBXファイル読み込み機能を追加する
...
デバッグ出力を見れば分かりますが、オークの目玉は`_Eyes`、牙は`_HighTheeth`と`_LowTheeth`というように、確かに各パーツが別モデルになっています。そこで、これらのパーツも表示してみます。
