...
ソリューションエクスプローラーでプロジェクト名を右クリックし、「プロパティ」を表示してください。
...
インクルード文を変更したら、次は`MikkTSpace`クラスの定義全体を切り取ってください。
...
FBXファイルでは、「FBXノード」によってメッシュやスケルトンの親子階層を定義します。FBXノードはFBXファイルのデータ構造の中心的なオブジェクトで、メッシュやマテリアルはFBXノードに割り当てられます。
...
FBXインプリメンテーションを取得するには`GetImplementation`(ゲット・インプリメンテーション)関数を使います。なお、インプリメンテーションは「実装」という意味です。<br>
...
### 2.11 GetMaterialListメンバ関数を定義する
...
+        m->roughness =
+          std::clamp(1 - log2(static_cast<float>(specularPower)) / 12, 0.0001f, 1.0f);
+      }
     } // if FbxSufraceLambert
     materials.push_back(m);
   } // for i < fbxMaterialCount
...
フォンマテリアルからは「ラフネス」が計算できます。実際に記録されている値は「スペキュラ」パラメータなので、スペキュラ係数からラフネスを計算します。スペキュラ係数の計算とラフネスへの変換は、`Mesh.cpp`の`LoadMTL`関数のものを流用しています。

これで、マテリアルが読み込めるようになりました。<br>
それでは、作成した`GetMaterialList`メンバ関数使ってマテリアルを取得しましょう。
...
### 2.12 CreateFbxLoader関数を定義する

`FbxLoader`は`FbxLoader.cpp`内で定義されているため、そのままでは他のファイルにあるプログラムからは利用できません。

そこで、`FbxLoader`オブジェクトを作成する`CreateFbxLoader`関数の出番です。
`ConvertMesh`メンバ関数の定義の下に、次のプログラムを追加してください。
...
+  const MeshBuffer::TextureCallback& textureCallback)
+{
+  return std::make_shared<FbxLoader>(mikkTSpace, textureCallback);
+}
```

この関数は`FbxLoader`オブジェクトを作成し、そのアドレスを`FbxLoaderInterface`型のポインタとして返します。外部のプログラムは`FbxLoaderInterface`の仮想関数を通して
`FbxLoader`を利用するわけです。

このように、インターフェイスクラスと仮想関数を利用すると、メンバ変数のようなデータのほとんどをCPPファイルに隠しながら、それを他のプログラムから利用できるようになります。

### 2.13 MeshBufferクラスにFBXファイル読み込み機能を追加する
...
メッシュシーンの頂点データとインデックスデータは既にOpenGL用に変換済みなので、そのまま
`AddVertexData`メンバ関数に渡すことができます。

その後のスタティックメッシュを作成するプログラムは、`LoadOBJ`メンバ関数にあるスタ<br>
ティックメッシュを作成するプログラムとほとんど同じです。
...
### 2.14 EngineクラスにFBXファイル読み込み機能を追加する
...
プログラムが書けたらビルドして実行してください。オークの身体だけが表示されていたら成功です。なお、`Orc Shaman.fbx`のモデルサイズは少し大きすぎるため、スケールを`0.2`倍に設定しています。

<p align="center">
<img src="images/2024_04_result_0.jpg" width="45%" />
</p>

実際に`_OrcBase`モデルを表示してみると、`Orc Shaman.fbx`では胴体と目玉、牙が別パーツになっているのか表示されず、なんだか不気味な印象です。

デバッグ出力を見れば分かりますが、オークの目玉は`_Eyes`、牙は`_HighTheeth`と
`_LowTheeth`というように、確かに各パーツが別モデルになっています。<br>
そこで、これらのパーツも表示してみます。
...
-      fbxTest->staticMesh = engine.GetStaticMesh(
-        "Res/MeshData/OrcShaman/Orc Shaman.fbx:_OrcBase");
+      auto name = std::string("Res/MeshData/OrcShaman/Orc Shaman.fbx:") + meshName;
...
>一部のモデルでテクスチャが表示されないかもしれません。その場合、デバッグ出力を調べて足りないテクスチャを追加してください。
...
>**【2章のまとめ】**
>
>* FBX SDKでは、`FbxManager`クラスがすべての管理している。
>* FBXファイルの情報は`FbxNode`オブジェクトのツリー構造として記録されている。
>* FBXファイルのメッシュ情報は`FbxMesh`オブジェクトに記録されている。
>* 頂点座標とそれ以外のテクスチャ座標、法線ではデータの格納方法が異なる。
>* FBXファイルには多数のマテリアルタイプがあり、それぞれ対応する読み込み処理を書く必要がある。
