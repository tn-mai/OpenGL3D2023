...
それでは、本題であるモノクロ画像を読み込む機能を`LoadDDSData`関数に追加しましょう。まず、「DDSファイルがモノクロ画像かどうかを判定するための定数」を定義します。`dds.h`に次の定義を追加してください。
...
+constexpr uint32_t DDPF_FORMAT_MASK = 0x2'0043; // RGB+輝度+透明度

 /**
 * DDSファイルヘッダ
```

無圧縮のモノクロ画像では、追加した定義のうち`DDPF_LUMINANCE`(ディーディーエスピーエス・ルミナンス)だけがフラグに含まれます。

`DDPF_FORMAT_MASK`は`DDPF_RGB`, `DDPF_LUMINANCE`, `DDPF_ALPHA`を足した値になっています。`flags`メンバ変数と`DDPF_FORMAT_MASK`を論理和した結果が`DDPF_LUMINACE`と一致したら、モノクロ画像と判定できます。

`dds.cpp`を開き、`LoadDDS`関数の定義にあるFourCCを判定するプログラムの下に、次のプログラムを追加してください。
...
```diff
     ...
     gpuFormat = imageFormat;
+  } else if ((header.ddspf.flags & DDPF_FORMAT_MASK) == DDPF_LUMINANCE) {
+    // 無圧縮の単色
