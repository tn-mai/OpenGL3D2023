...
として定義されるベクトルです。とりあえず「頂点からまっすぐ上に向かう方向ベクトル」だと考えてください。
...
最初に「法線が設定されていない頂点」を見つけます。やり方はいろいろありますが、今回は`bool`型の配列を用意して、法線が設定されていない場合は`true`を設定する方法を使います。この方法は比較的簡単で分かりやすいと思います。
...
&emsp;`a×b = (ay*bz - az*by, az*bx - ax*bz, ax*by - ay*bx)`

さて、外積によって法線を求めるには、ポリゴン面のうち2辺が分からなくてはなりません。これは頂点座標から計算できます。「法線を計算」というコメントの下に、次のプログラムを追加してください。
...
外積によって得られたベクトルの長さは`|a||b|sinθ`です。しかし、法線ベクトルは「向きを表すベクトル」なので、単位ベクトルにしたほうが計算しやすくなって都合がいいです。そこで、各要素のベクトルの長さで割ることで正規化しています。
...
これで`FillMissingNormals`関数は完成です。早速この関数を呼び出しましょう。`LoadOBJ`メンバ関数にある「読み込んだデータをOpenGLで使えるデータに変換」するプログラムの下に、次のプログラムを追加してください。
...
続いて、フラグメントシェーダに法線を受け取る入力変数を加えましょう。`standard.frag`を開き、次のプログラムを追加してください。
...
続いて、フラグメントシェーダに明るさの計算を追加します。最初に、法線を受け取る`in`変数を加えましょう。`standard.frag`を開き、次のプログラムを追加してください。

次に、明るさの元になるライトのパラメータを定義します。コンピューター・グラフィックスでは、光を放つ物体のことを「ライト」あるいは「光源(こうげん)」と呼びます。

基本的な光源には「ポイント・ライト(点光源)」、「ディレクショナル・ライト(平行光源)」、「スポット・ライト(※日本語訳なし)」、「アンビエント・ライト(環境光)」の4種類があります。

すべてを使えるようにするには時間がかかるので、今回はとりあえず「ポイント・ライト」のみ対応させることにします。

「ポイントライト」は、「ろうそく」や「電球」のような「ある一点からすべての方向に光を放つ光源」を表現するためのライトです。

<p align="center">
<img src="images/09_directional_light_.png" width="33%" /><br>
[点光源のイメージ]
</p>
...
ユニフォーム変数に構造体や配列を指定すると、要素ごとにロケーション番号が割り振られます。つまり、最初のメンバである`pointhight::color`が`101`、2番目の
...
こんなに時間がかかるのでは、とてもゲームのようなリアルタイム性を求められる分野には使えません。そんなわけで、簡単な計算でそれっぽい見た目になる方法がいくつも考案されています。
...
続いて、フラグメントシェーダに「ワールド座標を受け取る入力変数」を追加します。`standard.frag`を開き、入力変数に次のプログラムを追加してください。
...
```diff
 ...
 // シェーダへの入力
-layout(location=0) in vec4 inColor; // 頂点色
+layout(location=1) in vec3 inPosition; // ワールド座標
 layout(location=1) in vec2 inTexcoord; // テクスチャ座標
 layout(location=2) in vec3 inNormal;   // 法線ベクトル
```
...
+
+  // ランバート反射では入射光の明るさ"cosθ"は半球全体に反射される
+  // cosθを半球積分した結果はπになるので、入射光は半球全体に1/πの明るさで反射されると考えられる
...
+  glProgramUniform3f(*progStandard3D, 102,
...
>光の向きや法線だけでなく、「方向を表すベクトル」はどんなものでも必ず単位ベクトルにしておきましょう。明るさと同じ理由で、計算が簡単になります。
...
<strong>【課題01】</strong>
以下のURLから<code>skull_highpoly_with_normal.obj</code>というOBJファイルをダウンロードして、プロジェクトの<code>Res/MeshData</code>フォルダに保存しなさい。
...
ダウンロードした`skull_highpoly_with_normal.obj`を読み込みます。`Engine.cpp`を開き、`Initialize`メンバ関数に次のプログラムを追加してください。

```diff
   // OBJファイルを読み込む
   meshBuffer->LoadOBJ("Res/MeshData/skull_highpoly.obj");
...
-  highpoly->staticMesh = engine.GetStaticMesh("Res/MeshData/skull/skull_highpoly.obj");
...
<p align="center">
<img src="images/09_result_0b.jpg" width="45%" />
</p>
...
<p align="center">
<img src="images/09_gamma_curve_0.png" width="33%" /> <img src="images/09_gamma_curve_1.png" width="33%" />
</p>

ガンマ補正があるため、ライトの明るさも`1/2.2`乗する必要があります。ただし、単純に`1/2.2`乗すると、画像(テクスチャ)の`1/2.2`乗と重なって本来より明るい映像になってしまいます。

そもそも、ライトの計算に使われるさまざまな式は、ガンマ補正などない状態を前提として作られています。そのため、`2.2`乗されていたり`1/2.2`乗されている数値が紛れ込んでしまうと、正しい明るさを計算できません。

そこで、画像の明るさを`2.2`乗して`1/2.2`乗を打ち消してから、ライトの計算を行います。ライトの計算が終わってフラグメントの明るさが決まったら、その明るさを`1/2.2`乗して出力します。これによって、モニタには物理的に正しい明るさで表示されます。
...
なお、GLSLの数学関数はベクトル型を受け取れるように拡張されています。引数にベクトル型を指定した場合、各要素に対して個別に数学関数が実行されます。
...
点光源から1mの距離にある1平方mの領域が受ける光の量は、距離2mにある4平方mの領域が受ける光の量と等しく、同様に距離3mにある9平方mの領域が受ける光の量とも等しくなります。

<p align="center">
<img src="images/09_inverse_square_law.png" width="33%" />
</p>

上の図から分かるように、距離が遠くなるほど同じ光の量で照らすべき領域が拡大するため、単位面積あたりの光の量は低下します。

距離による明るさを計算するために、ライトまでの距離を計算しましょう。ベクトル`(x, y, z)`があるとすると、その長さは以下の式で求められます。

&emsp;√(x^2 + y^2 + z^2)

`standard.frag`を開き、ランバート反射を計算するプログラムに次のプログラムを追加してください。
...
`sqrDistance`(スクエア・ディスタンス、「距離の2乗」という意味)は「光源までの距離の2乗」を表します。
...
ライトの明るさを強くすると、かなり遠くまでライトが影響していることが分かります。逆２乗の法則が示すように、現実の光源は無限に影響を及ぼします。

もっとも、これは理論上のことです。現実の光子(こうし)は経路上のさまざまな物体や重力場の影響を受けるからです。そのため、特に明るさの弱い光源から放たれた光子が、無限の距離を旅することはほとんどありえません。

しかし、現在の計算式ではこれらの影響を完全に無視しており、壁などの遮蔽物(しゃへいぶつ)を貫通してどこまでも光が到達しています。これは現実にはありえないことです。

<p align="center">
<img src="images/09_result_4a.jpg" width="45%" />
</p>

問題は、遮蔽物の計算が難しいということです。そのため、多くのゲームでは一部の重要なライトに限って遮蔽を計算しています。それ以外のライトの光は壁を貫通しますが、光が届く距離を制限することで貫通が目立たないように工夫されています。

本テキストでも、貫通が目立たないように光が届く距離を設定することにします。
...
それでは`radius`の値を使って範囲を制限しましょう。これは物理的な根拠に乏しい処理である以上、少なくとも見た目が自然になるように工夫する必要があります。

今回はUnreal Engine 4やFrostbite 3で採用されている、以下の式を使うことにしました。

<p align="center">
<img src="images/09_falloff_function.png" width="40%" />
</p>

>式の詳細は以下のURLを参照<br>
>Unreal Engine 4: `https://blog.selfshadow.com/publications/s2013-shading-course/karis/s2013_pbs_epic_notes_v2.pdf`<br>
>Frostbite 3: `https://media.contentapi.ea.com/content/dam/eacom/frostbite/files/course-notes-moving-frostbite-to-pbr-v2.pdf`

この式を選択した理由について、上記ドキュメントの著者Brian Karis(ブライアン・カリス)氏は「光の影響の大部分が比較的影響を受けず、同時にゼロへのソフトな遷移を提供するように、範囲を制限した逆二乗関数を選択しました。」
と述べています。

というわけで、この式を使って光の到達距離を制限します。逆２乗の法則を使うプログラムの下に、次のプログラムを追加してください。
...
>**【1章のまとめ】**
>
>* OBJファイルの頂点には、頂点座標、テクスチャ座標、法線という最大3つのデータがある。
>* 法線は「曲面に接する平面に垂直な直線」で、ベクトルとして表現することができる。
>* 平面上の点の明るさは、法線と光の向きの内積で表すことができる。これは「ランバート反射」と呼ばれる。
...
ところで、これまで色だけはベクトル型ではなく配列を使っていました。これは単純に最初は必要なかったので`vec4`型を定義していなかったからですが、そろそろ`vec4`も定義しておくべきでしょう。
...
`StaticMesh`構造体にマテリアルの配列を持たせ、`DrawParams`構造体でマテリアル配列のインデックスを指定することで、描画パラメータごとに異なるマテリアル表現が可能となります。
