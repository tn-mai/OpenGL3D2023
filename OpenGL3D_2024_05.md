...
+    mat4 initialGlobalTransform; // 座標変換行列の初期値
+  };
+
...
続いて、初期座標変換行列を計算します。ここは、FBX SDKドキュメントの`How transformation matrices are computed`、というタイトルのページに書かれている説明を参考に作成しました。
...
+    joint.initialGlobalTransform =
+      ToMat4(fbxNode->EvaluateGlobalTransform() * matMeshNodePivot);
+
...
+    // モデル行列にジョイント行列を合成
+    mat4 matJointTransform = GetJointMatrix(e.jointId);
...
+      if (hasLight) {
+        glProgramUniform1i(*progSkeletal, locLightCount, activeLightCount);
+      }
...
```diff
   ...
   for (const auto& e : meshSet->staticMeshes) {
     // モデル行列にジョイント行列を合成
-    mat4 matJointTransform = meshSet->joints[e.jointId].initialGlobalTransform;
-    matJointTransform = matTransform * matJointTransform;
...
プログラムが書けたらビルドして実行してください。ノード行列を利用する前と同じように、ベルトの骨だけがずれて表示されていたら成功です。
...
また、3Dツールなどで確認すると、ベルトの骨以外のパーツはY方向に約3.4m移動しているようです。ということは、79番と80番以外のジョイントを下に3.4m移動させれば、ズレが解消されるはずです。
...
プログラムが書けたらビルドして実行してください。地面に埋まらず、ベルトの骨もちゃんとベルトの位置に表示されていたら成功です。
