...
それでは、ゲームエンジンに作成したシーンを設定しましょう。`Main.cpp`を開き、`MainGameScene.h`をインクルードしてください。
...
この関数は、`glDrawElementsInstanced`関数に「インデックス0とみなす頂点データの位置」を追加したものです。これを「ベースバーテックス」と呼びます。
...
これで、ゲームオブジェクトの図形番号を変えれば、好きな図形を描画できるようになりました。
...
誤ってHTML形式でダウンロードしないように注意してください。HTML形式かどうかは、実際にファイルを開いてみるか、ファイルサイズを見れば判断できるでしょう。
...
そこで、壁を配置するY座標を0に変更して「ずれ」を解消しなさい。
</pre>
...
とりあえず衝突判定で使う`Collision.h`をインクルードしましょう。`MainGameScene.cpp`を開き、次のプログラムを追加してください。
...
次に周囲のマス目を調べます。まず、「調べるマス目の位置」を配列にします。配列名は`neighborOffsets`(ネイバー・オフセッツ, `neighbor`は「隣の」、`offset`は「差分、相殺」という意味なので、「隣のマスまでの差分」のような意味)とします。
