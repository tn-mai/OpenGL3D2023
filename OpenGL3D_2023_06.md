...
+  auto message = engine->Create<GameObject>("goal message");
...
これによって、UIの配置や大きさが制御しやすくなるはずです。代償として「ゲームオブジェクトにZ座標を設定しても無視される」という、やや直感的ではない設計になってしまいますが、トレードオフとして受け入れることにします。
...
これで、視野角を制御する機能を追加することができました。

### 2.9 UIのZ座標を設定する

視野角を制御する機能を使って、UI画像のZ座標を設定しましょう。`UILayout.cpp`を開き、`Update`メンバ関数の定義に次のプログラムを追加してください。
...
プログラムが書けたらビルドして実行してください。ゴール画像が小さくなっていたら成功です。
