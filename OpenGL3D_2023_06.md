...
さて、コライダーはコンポーネントの派生クラスとして定義します。プロジェクトの`Src/Engine`フォルダに`Collider.h`(コライダー・エイチ)という名前のヘッダファイルを追加してください。
...
コライダーは衝突判定を行うたびに必要となりますが、コンポーネント配列からコライダーを検索するのは時間がかかります。そこで、コライダーを他のコンポーネントと分けて管理します。これによって、コライダーの検索を不要にしています。
...
演算子オーバーロードとは、関数名として「`operator`(オペレータ)と演算子」を指定することで、組み込み型と同様の計算を可能にする「C++の機能」のことです。上記のプログラムでは加算と乗算の2つの演算子をオーバーロードしています。

それでは、これらの演算子を使ってコライダーの座標変換プログラムを修正しましょう。`Engine.cpp`を開き、コライダーの座標変換プログラムを次のように変更してください。
...
この衝突判定プログラムはおおよそ完成していますが、ひとつだけ考慮されていない場合があります。それは、「`OnCollision`の結果、ゲームオブジェクトが削除された場合」です。

ゲームオブジェクトが削除されたなら、もはや衝突判定を続ける必要はないはずです。そこで、ゲームオブジェクトが削除されていたらコライダーの衝突判定ループを終了させます。

ただ、これは2重forなので`break`を書くだけではループを終了させられません。フラグ変数や`goto`を使えば解決できますが、どちらもあまり良い解決策とは言えません。

また、もうひとつの方法として「ループ部分を関数に分離する」というやり方があります。今回のプログラムの場合、そもそものループが深く複雑になりすぎているため、関数に分けるのは良い選択です。

ループが深くなってしまう原因は「複数の処理をひとつにまとめてしまっている」ことにあります。ここでは以下の2つの処理がまとめられていると考えられます。
...
```diff
   ...
   void UpdateGameObject(float deltaTime);
   void HandleGameObjectCollision();
+  bool HandleWorldColliderCollision(
...
そこで、プレイヤーオブジェクトを`MainGameScene`に移動させましょう。まずは管理用の変数を追加します。`MainGameScene.h`を開き、`MainGameScene`クラスの定義に次のプログラムを追加してください。
...
<strong>【課題01】</strong>
<code>Engine.cpp</code>から<code>PlayerComponent.h</code>のインクルード文を切り取り、<code>MainGameScene.cpp</code>に貼り付けて、エラーを解消しなさい。
</pre>
...
-  auto player = Create<GameObject>("camera", { 0, 10, 0 });
+  // プレイヤーオブジェクトを追加
+  player = Create<GameObject>("camera", { 0, 10, 0 });
   player->AddComponent<PlayerComponent>();
...
作成した画像ファイルをプロジェクトの<code>Res</code>フォルダにダウンロードし、TGA形式に変換しなさい。ファイル名は<code>you_escaped.tga</code>としなさい。
</pre>
...
+  GameObjectPtr message;  // メッセージ用オブジェクト
+};
+
...
+  message = engine->Create<GameObject>("goal message");
+  message->position = GetOwner()->position;
+  message->texColor = std::make_shared<Texture>("Res/you_escaped.tga");
...
マップデータのゴール地点は文字`G`で表されます。`Initialize`メンバ関数にある、マップデータを読み込むプログラムに、次のプログラムを追加してください。
...
+      // ゴール判定オブジェクトを作成
+      if (s[x] == 'G') {
+        auto goal = engine.Create<GameObject>("goal", {
+          (static_cast<float>(x) + 0.5f) * squareSize, 0,
+          (static_cast<float>(y) + 0.5f) * squareSize });
...
ただし、縦横比は画像によって異なるため、固定の縦横比は使えません。そこで、縦横比をプログラムで計算します。`GoalEvent.cpp`を開き、`OnCollision`メンバ関数の定義に次のプログラムを追加してください。
...
+    static_cast<float>(message->texColor->GetWidth()) /
+    static_cast<float>(message->texColor->GetHeight());
+  message->scale = { baseScale * aspectRatio, baseScale, 1 };
 }
```

縦横比は「画像の幅÷画像の高さ」で求めます。基本の拡大率`baseScale`(ベース・スケール)は画像に合わせて変える必要があります。うまく調整してください。

プログラムが書けたらビルドして実行してください。ゴール画像が読みやすいサイズになっていたら成功です。
...
ところで「メッセージが上下さかさまに表示される」という症状が出てはいませんか？

これは、Visual Studioを使って画像をTGAファイルに変換すると、画像の上下が逆になるからです。というのも、実はTGAファイルは「画像データの格納方向を選べる」のです。
...
ユーザー体験を損なわないために、ゴール画像のような演出は「カメラの向きにかかわらず、常に画面の同じ位置に、同じ大きさで表示」したいものです。
...
このように、ゲームオブジェクトの向きと座標をうまく調整することで、カメラにくっついて動くゲームオブジェクトが作れます。

>UILayoutの動作は、UnityのCanvasのレンダーモードを`Screen Space - Camera`に設定した状態と似ています(まったく同じではありませんが)。
...
2章の最後で、ゴール画像がそれらしく表示されるようにしました。しかし、迷路を動き回ると「画像が壁を貫通して隠れてしまう」ことがあります。板ポリゴンに貼り付けた画像でしかない以上、隠れてしまうのは仕方ありません。
...
>全てのマクロ定数は`https://registry.khronos.org/OpenGL-Refpages/gl4/html/glBlendFunc.xhtml`に掲載されています。
