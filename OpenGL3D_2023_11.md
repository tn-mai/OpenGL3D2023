...
霧ジェネレータの座標は、マップに合わせて変更してください。名前に指定した`3x2`(さん・エックスt・に)というのは、霧を発生させる範囲を表しています。
...
>* 乱数を生成するには`mt19937`クラスを使う。指定した範囲の乱数を作るには、`uniform_real_distribution`クラスを組み合わせて使う。
...
その前に、ヘッダファイルをインクルードしましょう。`Engine.cpp`を開き、`EasyAudio.h`をインクルードしてください。
...
ヘッダファイルに変数や定数を定義すると、インクルード先のCPPファイルごとにコピーが作られます。
...
<pre class="tnmai_assignment">
<strong>【課題17】</strong>
ドアが開くときに<code>SE::doorOpen</code>を再生するプログラムを追加しなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題18】</strong>
<code>SimpleLever</code>コンポーネントに、レバーがクリックされたときに<code>SE::leverMove</code>を再生するプログラムを追加しなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題19】</strong>
ゲームクリアおよびゲームオーバーになったとき、BGMを停止し、<code>SE::goalEvent</code>、<code>SE::gameOverEvent</code>を再生するプログラムを追加しなさい
</pre>

<pre class="tnmai_assignment">
<strong>【課題20】</strong>
ゲームクリアおよびゲームオーバーにおいて、「戻るボタン」をクリックしたときに<code>SE::buttonClick</code>を再生するプログラムを追加しなさい。
</pre>

>**【3章のまとめ】**
>
>* 音声を選ぶときは「ゲームの雰囲気に合うこと」、「他の就職活動をしている人が使っていそうな音声は避けること」、の2点に気をつける。
